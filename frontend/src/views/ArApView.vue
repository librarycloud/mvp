...
async function saveSettlement() {
  ...
  try {
    const suffix = mode.value === "receivable" ? "receipts" : "payments";
    await api.post(`/ar-ap/${mode.value === "receivable" ? "receivables" : "payables"}/${activeDocument.value.id}/${suffix}`, { ...settlement.value, bankTransactionId: settlement.value.bankTransactionId || undefined });
    settlementDialog.value = false;
    await load();
    ElMessage.success("核销完成，凭证已记账");
  } finally {
...
async function completeFollowUp(row: any) {
  let result: any;
  try {
    result = await ElMessageBox.prompt("请输入跟进结果", "完成跟进", { inputValue: row.result ?? "", inputValidator: (value) => Boolean(value.trim()) || "结果不能为空" });
  } catch {
    return;
  }
  await api.post(`/ar-ap/follow-ups/${row.id}/complete`, { result: result.value });
  await loadSupport();
  ElMessage.success("跟进已完成");
}
async function cancelLatest(row: any) {
  const latest = (row.settlements ?? []).find((item: any) => item.status === 2);
  if (!latest) return;
  let res: any;
  try {
    res = await ElMessageBox.prompt("请输入撤销原因", "撤销核销", { inputPattern: /\S+/, inputErrorMessage: "原因不能为空" });
  } catch {
    return;
  }
  const { value } = res;
  const prefix = mode.value === "receivable" ? "receivable-settlements" : "payable-settlements";
  await api.post(`/ar-ap/${prefix}/${latest.id}/cancel`, { reason: value });
  await load();
  ElMessage.success("核销已撤销");
}
  ...
  <div class="page-grid">
    ...
    <div class="two-column">
      <el-card shadow="never">
        ...
        <el-table table-layout="auto" v-loading="loading" :data="balances" size="small">
      ...
      <el-card shadow="never">
        ...
        <el-table v-if="agingViewType === 'list'" table-layout="auto" v-loading="loading" :data="aging" size="small">
