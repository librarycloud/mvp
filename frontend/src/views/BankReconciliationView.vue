...
<template><div class="page-grid">
  ...
  <el-dialog v-model="createVisible" title="建立银行对账单" width="520px"><el-form label-width="130px"><el-form-item label="会计期间"><el-select v-model="form.periodId" style="width:100%" @change="() => syncCreateOpeningBalance(false)"><el-option v-for="row in periods" :key="row.id" :label="row.periodCode" :value="row.id"/></el-select></el-form-item><el-form-item label="银行科目"><el-select v-model="form.bankAccountId" filterable style="width:100%" @change="() => syncCreateOpeningBalance(false)"><el-option v-for="row in accounts" :key="row.id" :label="`${row.code} ${row.name}`" :value="row.id"/></el-select></el-form-item><el-form-item label="银行期初余额"><div style="display:flex;gap:8px;width:100%"><el-input v-model="form.statementOpeningBalance"/><el-button :loading="loadingPreviousCreate" @click="syncCreateOpeningBalance(true)">带入上期期末</el-button></div><div v-if="previousCreateInfo?.hasPrevious" class="form-tip" style="color:var(--el-text-color-secondary);font-size:12px;margin-top:4px">上期（{{previousCreateInfo.periodCode}}）银行期末余额为 ¥{{previousCreateInfo.statementClosingBalance}}</div><div v-else-if="previousCreateInfo && !previousCreateInfo.hasPrevious" class="form-tip" style="color:var(--el-text-color-placeholder);font-size:12px;margin-top:4px">未查询到上期对账单，可手工输入期初余额</div></el-form-item><el-form-item label="银行期末余额"><el-input v-model="form.statementClosingBalance"/></el-form-item><el-form-item label="备注"><el-input v-model="form.remark"/></el-form-item></el-form><template #footer><el-button @click="createVisible=false">取消</el-button><el-button type="primary" @click="create">建立</el-button></template></el-dialog>
  <el-dialog v-model="editVisible" title="编辑银行对账单" width="520px"><el-form label-width="130px"><el-form-item label="会计期间"><el-input :model-value="active?.period?.periodCode" disabled/></el-form-item><el-form-item label="银行科目"><el-input :model-value="active ? `${active.bankAccount.code} ${active.bankAccount.name}` : ''" disabled/></el-form-item><el-form-item label="银行期初余额"><div style="display:flex;gap:8px;width:100%"><el-input v-model="editForm.statementOpeningBalance"/><el-button :loading="loadingPreviousEdit" @click="applyPreviousToEdit">带入上期期末</el-button></div><div v-if="previousEditInfo?.hasPrevious" class="form-tip" style="color:var(--el-text-color-secondary);font-size:12px;margin-top:4px">上期（{{previousEditInfo.periodCode}}）银行期末余额为 ¥{{previousEditInfo.statementClosingBalance}}</div><div v-else-if="previousEditInfo && !previousEditInfo.hasPrevious" class="form-tip" style="color:var(--el-text-color-placeholder);font-size:12px;margin-top:4px">未查询到上期对账单</div></el-form-item><el-form-item label="银行期末余额"><el-input v-model="editForm.statementClosingBalance"/></el-form-item><el-form-item label="备注"><el-input v-model="editForm.remark" type="textarea" :rows="3" maxlength="500" show-word-limit/></el-form-item></el-form><template #footer><el-button @click="editVisible=false">取消</el-button><el-button type="primary" :loading="editSaving" @click="saveEdit">保存</el-button></template></el-dialog>
