...
export class PrismaPeriodCloseChecker implements AccountingPeriodCloseChecker {
  ...
  async inspect(period: AccountingPeriodRecord) {
    const [pendingVouchers, totals, unmatchedBank, reconciliations, assets, employees, salaries, reimbursements, reports, priorOpenPeriod] = await Promise.all([
      ...
      this.prisma.accountingPeriod?.findFirst ? this.prisma.accountingPeriod.findFirst({ where: { deletedAt: null, status: 0, OR: [{ year: { lt: period.year } }, { year: period.year, month: { lt: period.month } }] }, orderBy: [{ year: "asc" }, { month: "asc" }], select: { periodCode: true } }) : Promise.resolve(null),
    ]);
    ...
    const unclosedProfitLoss = typeof this.prisma.voucherEntry.groupBy === "function"
      ? await this.prisma.voucherEntry.groupBy({
          by: ["accountId"],
          where: { deletedAt: null, voucher: { periodId: period.id, status: VOUCHER_STATUS.POSTED, deletedAt: null }, account: { category: "PROFIT_AND_LOSS", deletedAt: null } },
          _sum: { debitAmount: true, creditAmount: true },
        })
      : [];
    const unclosedProfitLossCount = unclosedProfitLoss.filter(
      item => !(item._sum.debitAmount ?? ZERO).equals(item._sum.creditAmount ?? ZERO)
    ).length;
