...
export async function buildApp(options: BuildAppOptions = {}) {
  ...
  const fixedAssetController = new FixedAssetController(new FixedAssetService(modulePrisma, new AccountingPeriodService(accountingPeriodRepository)));
  const arApController = new ArApController(new ArApService(modulePrisma, new AccountingPeriodService(accountingPeriodRepository)));
  const salaryController = new SalaryController(new SalaryService(modulePrisma, new AccountingPeriodService(accountingPeriodRepository)));
  const yearEndClosingController = new YearEndClosingController(new YearEndClosingService(modulePrisma));
  const depreciationController = new DepreciationController(new DepreciationService(modulePrisma, new AccountingPeriodService(accountingPeriodRepository)));
  ...
  const reimbursementController = new ReimbursementController(new ReimbursementService(modulePrisma, new AccountingPeriodService(accountingPeriodRepository), budgetService));
  ...
  const taxController = new TaxController(new TaxService(modulePrisma));
