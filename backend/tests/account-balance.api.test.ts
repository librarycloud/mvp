...
describe("account balance API", () => {
  ...
  it("returns the account balance table to authenticated users", async () => {
    ...
    const app = await buildApp({ config, authRepository, accountBalanceRepository: new FakeAccountBalanceRepository(), logger: false });
    apps.push(app);
    const login = await app.inject({ method: "POST", url: "/api/v1/auth/login", payload: { username: "user", password: "correct-password" } });
    const response = await app.inject({
      method: "GET", url: "/api/v1/account-balances?startDate=2026-07-01&endDate=2026-07-31&includeZero=false",
      headers: { authorization: `Bearer ${login.json().data.accessToken as string}` },
    });
    expect(response.statusCode).toBe(200);
    expect(response.json()).toMatchObject({ success: true, data: { rows: [] } });
  });
});
