...
describe("detail ledger API", () => {
  const apps: Awaited<ReturnType<typeof buildApp>>[] = [];
  afterEach(async () => Promise.all(apps.splice(0).map((app) => app.close())));

  it("returns authenticated detail ledger output", async () => {
    ...
    const app = await buildApp({ config, authRepository, detailLedgerRepository: new FakeDetailLedgerRepository(), logger: false });
    apps.push(app);
    ...
    const response = await app.inject({
      method: "GET",
      url: "/api/v1/detail-ledger?accountId=701&startDate=2026-07-01&endDate=2026-07-31&auxiliaryKey=department&auxiliaryValue=%E8%A1%8C%E6%94%BF",
      headers: { authorization: `Bearer ${login.json().data.accessToken as string}` },
    });
    expect(response.statusCode).toBe(200);
    expect(response.json()).toMatchObject({ success: true, data: { account: { code: "6602" }, auxiliary: { key: "department", value: "行政" } } });
  });
});
